...
parser.add_argument('--dataset', type=str, default='meituan002', help='dataset name')
...
# 加载 IID 数据
iid_users = np.load('../data/meituan002/' + 'demo6_iid_users.npy', allow_pickle=True)
# 加载 OOD 数据
ood_users = np.load('../data/meituan002/' + 'demo6_ood_users.npy', allow_pickle=True)
...
if args.ood_finetune:
    ...
    if args.model_name == 'COR':
        model = models002.COR(mlp_q_dims, mlp_p1_dims, mlp_p2_dims, mlp_p3_dims, \
                           adj, E1_size, args.dropout, args.bn, args.sample_freq, args.regs,
                           args.act_function).to(device)
    elif args.model_name == 'COR_G':
        model = models002.COR_G(mlp_q_dims, mlp_p1_1_dims, mlp_p1_2_dims, mlp_p2_dims, mlp_p3_dims, \
                             adj, E1_size, args.dropout, args.bn, args.sample_freq, args.regs,
                             args.act_function).to(device)

optimizer = optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.wd)
criterion = models002.loss_function
